# House Music Generator - Implementation Guide

This document contains all the code needed to add a procedural house music generator to your game.
...
src/
  rendering/
    backgroundmusic.js   <- Main house music generator
    audio.js             <- Audio manager wrapper
test-audio.html          <- Standalone test page (optional)
```

---

## File 1: `src/rendering/backgroundmusic.js`
...
## File 3: `test-audio.html` (Optional - for standalone testing)

Place this in your project root to test without a dev server:
...
<head>
  ...
  <title>House Music Generator Test</title>
  <style>
    body {
    ...
    #stopBtn { background: #333; color: white; }
  </style>
</head>
<body>
  <h1>House Music Generator</h1>
  <p>Procedural house music using Web Audio API</p>
  <div>
    <button id="startBtn" onclick="startMusic()">START</button>
    <button id="stopBtn" onclick="stopMusic()">STOP</button>
  </div>

  <script type="module">
    import HouseMusic from './src/rendering/backgroundmusic.js';
    window.HouseMusic = HouseMusic;
    window.startMusic = () => HouseMusic.start();
    window.stopMusic = () => HouseMusic.stop();
    console.log('House Music Generator loaded! Click START to begin.');
  </script>
</body>
</html>
```
...
import audioManager from './rendering/audio.js';
...
audioManager.toggleMusicElement('kick');
audioManager.toggleMusicElement('bass');

// Trigger filter sweep effect
audioManager.triggerFilterSweep(1);  // 1 = up, -1 = down
```
...
| Element | Description |
|---------|-------------|
| **Kick** | Four-on-the-floor at 124 BPM with click transient |
| **Hi-hat** | Offbeat 8ths with occasional open hats and ghost 16ths |
| **Clap** | Layered noise bursts on beats 2 and 4, heavy reverb |
| **Shaker** | Constant 16ths with accent pattern |
| **Conga** | Syncopated Latin-style pattern |
| **Bass** | Sub + harmonic layer, syncopated, follows chord progression |
| **Chords** | Rhodes-style FM synthesis 7th chord stabs |
| **Pads** | Warm detuned oscillators with slow attack |
| **Effects** | Convolution reverb, dotted-eighth delay, filter sweeps |

**Arrangement:** 16-section cycle with gradual layering, breakdowns, and drops. Elements wax and wane for endless variation.
